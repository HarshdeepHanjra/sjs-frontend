...
const InternshipPayment = () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (newStatus !== paymentStatus) {
          ...
          if (newStatus === "approved") {
            toast.success("Payment verified! You are now enrolled in the internship.");
            setTimeout(() => navigate("/my-internships"), 3000);
          } else if (newStatus === "declined") {
            toast.error("Payment verification failed. Please contact support.");
          }
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      const response = await api.post("/api/internships/create-order", {
        internship_id: internship.id,
        internship_title: internship.title,
        amount: internship.fee,
        phone: userData.phone || "",
  ...
  // ✅ Step 2: Upload Screenshot & Submit for Verification
  const handleUploadAndVerify = async () => {
    if (!screenshot) {
      toast.error("Please upload payment screenshot");
      return;
    ...
    try {
      // Upload screenshot
      const uploadResponse = await api.post("/api/internships/upload-screenshot", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      ...
      if (uploadResponse.data.success) {
        ...
        const verifyResponse = await api.post("/api/internships/verify-payment", {
          order_id: orderId,
          screenshot_url: screenshotUrl,
        });
        ...
        if (verifyResponse.data.success) {
          ...
          localStorage.setItem("pendingInternshipVerification", JSON.stringify(pendingData));

          toast.success("Payment verification submitted! Admin will review within 24 hours.");
        } else {
          toast.error(verifyResponse.data.error || "Verification failed");
        }
      } else {
  ...
  const handleFreeEnroll = async () => {
    ...
    try {
      ...
      const response = await api.post("/api/internships/enroll-free", {
        internship_id: internship.id,
        internship_title: internship.title,
      }, config);
      ...
      <div className="min-h-screen bg-gray-50 py-16">
        <div className="container mx-auto px-4 max-w-md">
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            ...
            <p className="text-gray-600 mb-4">You are now enrolled in {internship?.title}.</p>
            {adminNotes && (
              <div className="bg-green-50 p-3 rounded-lg mb-4 text-sm">
                <p className="text-green-700">{adminNotes}</p>
              </div>
            )}
            <button onClick={() => navigate("/my-internships")} className="w-full bg-primary-600 text-white px-6 py-3 rounded-lg hover:bg-primary-700">
              Go to My Internships
            </button>
          </div>
        </div>
      ...
      <div className="min-h-screen bg-gray-50 py-16">
        <div className="container mx-auto px-4 max-w-md">
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            ...
            <button onClick={() => window.location.reload()} className="w-full bg-primary-600 text-white py-3 rounded-lg">
              Try Again
            </button>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-16">
        <div className="container mx-auto px-4 max-w-md">
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            <div className="text-green-600 text-6xl mb-4">🎓</div>
            <h2 className="text-2xl font-bold text-gray-800 mb-2">Free Internship!</h2>
            <p className="text-gray-600 mb-6">This internship is completely free. Click below to enroll instantly.</p>
            <button onClick={handleFreeEnroll} disabled={verifying} className="w-full bg-green-600 text-white py-3 rounded-lg hover:bg-green-700 disabled:opacity-50">
              {verifying ? <FaSpinner className="animate-spin inline mr-2" /> : "Enroll Now for Free →"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-gray-50 py-16">
        <div className="container mx-auto px-4 max-w-md">
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            ...
            <p className="text-gray-600 mb-4">Your payment proof has been submitted. Our team will verify within 24 hours.</p>
            <button onClick={checkStatus} disabled={checkingStatus} className="w-full bg-primary-600 text-white py-3 rounded-lg hover:bg-primary-700 disabled:opacity-50 flex items-center justify-center gap-2">
              {checkingStatus ? <FaSpinner className="animate-spin" /> : <FaEye />}
              {checkingStatus ? "Checking..." : "Check Status"}
            </button>
            <button onClick={() => navigate("/internship")} className="w-full mt-3 bg-gray-300 text-gray-800 py-3 rounded-lg hover:bg-gray-400">
              Browse More Internships
            </button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-16">
      <div className="container mx-auto px-4 max-w-2xl">
        <button onClick={() => navigate("/internship")} className="flex items-center gap-2 text-primary-600 mb-6">
          <FaArrowLeft /> Back to Internships
        </button>
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="p-6 text-center border-b">
            <p className="text-gray-500 mb-2">Amount to Pay</p>
            <p className="text-4xl font-bold text-gray-800">₹{internship.fee?.toLocaleString()}</p>
          </div>

          {!orderId ? (
            <div className="p-6 text-center">
              <p className="text-gray-600 mb-4">Click below to create your payment order</p>
              <button onClick={handleCreateOrder} disabled={verifying} className="w-full bg-primary-600 text-white py-3 rounded-lg hover:bg-primary-700 disabled:opacity-50">
                {verifying ? <FaSpinner className="animate-spin inline mr-2" /> : "Proceed to Payment"}
              </button>
            </div>
          ) : (
            <div className="p-6">
              <h2 className="text-xl font-bold text-center mb-4">Pay with UPI (Instant Access)</h2>

              {/* QR Code */}
              <div className="text-center mb-6">
                <p className="text-gray-600 mb-3">Scan QR code with any UPI app</p>
                <div className="bg-white p-4 rounded-2xl shadow-lg border-2 border-primary-200 inline-block">
                  <img src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(YOUR_UPI_ID)}`} alt="UPI QR Code" className="w-64 h-64 object-contain" />
                </div>
                <button onClick={handleDownloadQR} className="mt-3 text-primary-600 text-sm flex items-center gap-1 mx-auto">
                  <FaDownload /> Download QR Code
                </button>
              </div>
              ...
              <div className="text-center mb-6">
                <p className="text-gray-600 mb-2">Or pay using UPI ID</p>
                <div className="flex items-center justify-center gap-3 flex-wrap">
                  <code className="bg-gray-100 px-4 py-2 rounded-lg font-mono text-primary-600 text-lg">{YOUR_UPI_ID}</code>
                  <button onClick={handleCopyUPI} className="bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-lg transition flex items-center gap-2">
                    {copied ? <FaCheckCircle /> : <FaCopy />}
                    {copied ? "Copied!" : "Copy"}
                  </button>
                </div>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center mb-6">
                ...
                {screenshot && (
                  <div className="mt-3">
                    ...
                    {screenshotPreview && <img src={screenshotPreview} alt="Preview" className="mt-2 max-h-32 mx-auto rounded border" />}
                  </div>
                )}
              </div>

              {/* Verify Button */}
              <button onClick={handleUploadAndVerify} disabled={verifying || !screenshot} className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-lg transition disabled:opacity-50 mb-3">
                {verifying ? <FaSpinner className="animate-spin inline mr-2" /> : "✅ Submit for Verification"}
              </button>
              ...
              <div className="bg-blue-50 rounded-xl p-4">
                ...
                <ol className="text-sm text-gray-700 space-y-1 list-decimal list-inside">
                  ...
                  <li>Click "Submit for Verification"</li>
                  <li>Admin will verify within 24 hours</li>
                  <li>After approval, internship will appear in "My Internships"</li>
                </ol>
              </div>
            </div>
          )}
          ...
          <div className="p-6 text-center border-t">
            <a href="https://wa.me/918950026639?text=Hello%20SJS%20Academy%2C%20I%20need%20help%20with%20my%20internship%20payment." target="_blank" rel="noopener noreferrer" className="block">
              <button className="w-full flex items-center justify-center gap-2 border border-green-500 text-green-600 py-2 rounded-lg hover:bg-green-50 transition">
                <FaWhatsapp /> Need Help? WhatsApp Us
              </button>
            </a>
